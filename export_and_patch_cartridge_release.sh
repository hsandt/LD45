...
rel_p8_folder="${cartridge_release_name}_cartridges"
rel_png_folder="${cartridge_release_name}_png_cartridges"
rel_bin_folder="${cartridge_release_name}.bin"
rel_web_folder="${cartridge_release_name}_web"
...
adapt_for_png_cmd="python3.6 \"$picoboots_scripts_path/adapt_for_png.py\" "${export_folder}/${cartridge_basename}*.p8
...
# there is only one code cartridge to check for PNG export failure
if [[ ! -f "${png_folder}/${cartridge_basename}.p8.png" ]]; then
  echo ""
...
patch_bin_cmd="\"$picoboots_scripts_path/patch_pico8_runtime.sh\" --inplace \"$bin_folder\" \"$cartridge_release_name\""
echo "> $patch_bin_cmd"
...
html_filepath="${web_folder}/${cartridge_release_name}.html"
mv "$html_filepath" "${web_folder}/index.html"

# Patch the HTML export in-place with 4x_token, fast_reload
js_filepath="${web_folder}/${cartridge_release_name}.js"
...
pushd "${export_folder}"
  ...
  rm -f "${cartridge_release_name}_cartridges.zip"
  zip -r "${cartridge_release_name}_cartridges.zip" "$rel_p8_folder"

  # PNG cartridges archive (delete existing one to be safe)
  rm -f "${cartridge_release_name}_png_cartridges.zip"
  zip -r "${cartridge_release_name}_png_cartridges.zip" "$rel_png_folder"
  ...
  rm -f "${cartridge_release_name}_web.zip"
  zip -r "${cartridge_release_name}_web.zip" "${cartridge_release_name}_web"
  ...
  pushd "${rel_bin_folder}"
    ...
    mv "linux" "${cartridge_release_name}_linux"
    ...
    zip -q "${cartridge_release_name}_linux.zip" "${cartridge_release_name}_linux/${cartridge_release_name}"


    # OSX archive

    # Replace the executable in the archive generated by PICO-8 export with the patched executable
    zip -q "${cartridge_release_name}_osx.zip" "${cartridge_release_name}.app/Contents/MacOS/${cartridge_release_name}"


    # Windows archive

    # Rename linux folder with full game name so our archive contains a self-explanatory folder as the initial archive
    mv "windows" "${cartridge_release_name}_windows"

    # To minimize operations, do not recreate the archive, just replace the executable in the archive generated by PICO-8 export
    # with the patched executable. We still get some warnings about "Local Version Needed To Extract does not match CD"
    # on other files, so make the operation quiet (-q)
    zip -q "${cartridge_release_name}_windows.zip" "${cartridge_release_name}_windows/${cartridge_release_name}.exe"
