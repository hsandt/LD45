...
# Run serialize cartridge with PICO-8 executable as headless
# Must be called after build script
# Usage: run_serialize.sh config [extra]
...
run_cmd="pico8 -x build/v${version}_${config}/${cartridge_stem}_${config}.p8 -screenshot_scale 4 -gif_scale 4 $@"

# Support UNIX platforms without gnome-terminal by checking if the command exists
# If you `reload.sh` the serialize, the separate terminal allows you to keep watching the program output,
